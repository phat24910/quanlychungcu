...
@Component({
  selector: 'app-phieu-thu-form',
  templateUrl: './phieu-thu-form.component.html',
  styleUrls: ['./phieu-thu-form.component.scss']
})
export class PhieuThuFormComponent implements OnInit {
  form!: FormGroup;
  saving = false;
  ...
  save(): void {
    ...
    this.svc.taoPhieuThu(payload).subscribe({
      next: (res: ApiResponse<any>) => {
        this.saving = false;
        if (res.isOk) {
          this.notification.success('Thành công', 'Tạo phiếu thu thành công');
          this.modalRef.close(true);
        } else {
          this.notification.error('Lỗi', res.errors?.[0]?.description || 'Tạo phiếu thu thất bại');
        }
      },
      error: () => {
        this.saving = false;
        this.notification.error('Lỗi', 'Tạo phiếu thu thất bại');
      }
    });
  }
